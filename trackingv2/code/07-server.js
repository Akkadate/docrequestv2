...
// ===============================================
// เพิ่มโค้ดนี้ใน server.js เพื่อเชื่อมต่อ Approval Workflow

// เพิ่มการ import ใหม่ (เพิ่มหลังจาก import เดิม)
const { addPoolToRequest } = require('./middleware/advisor');

// เปลี่ยนส่วนการ import routes (แทนที่โค้ดเดิม)
...
// ตรวจสอบว่ามี LINE notification service หรือไม่ (เดิม)
...
// เพิ่ม middleware สำหรับ pool (ใหม่ - เพิ่มก่อน routes)
app.use(addPoolToRequest(pool));

// ใช้งานเส้นทาง (อัปเดต)
...
// เพิ่ม endpoint สำหรับทดสอบ LINE notification (เดิม - ไม่เปลี่ยน)
...
// เส้นทางหลัก (เดิม - ไม่เปลี่ยน)
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'public/index.html'));
});

// Error handling middleware (เดิม - ไม่เปลี่ยน)
